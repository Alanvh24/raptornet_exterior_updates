...
import com.utmir.raptornet.dto.Docentes.CalificacionRemedialResponseDTO;
...
public class CalificacionService {
        ...
        public LocalDate calcularCincoDiasHabiles(LocalDate fechaInicio) {
                LocalDate fecha = fechaInicio;
                int diasAgregados = 0;
                while (diasAgregados < 5) {
                        fecha = fecha.plusDays(1);
                        // Verifica si el día es sábado o domingo y sáltalo
        ...
        public ResponseEntity<?> registrarCalificacionCondiciones(CalificacionDTO calificacionDTO) {
                AsignacionAsignatura asignacion = asignacionAsignaturaRepository
                ...
                if (Boolean.TRUE.equals(asignacion.getHabilitarCandado())) {
                        return registrarCalificacionSinCandados(calificacionDTO);
                } else {
                        return registrarCalificacion(calificacionDTO);
                }
        }
        ...
        public ResponseEntity<?> registrarCalificacion(CalificacionDTO calificacionDTO) {
                ...
                try {
                        ...
                        Optional<FechasUnidades> fechaUnidadOpt = fechasUnidadesRepository
                                        .findByUnidadesPorAsignaturaAndAsignacionAsignatura(unidadesPorAsignatura,
                                                        asignacionAsignatura);

                        // Comentar de aqui hacia abajo si se requiere habilitar calificaciones
                        if (fechaUnidadOpt.isPresent()) {
                                FechasUnidades fechaUnidad = fechaUnidadOpt.get();
                                LocalDate hoy = LocalDate.now();
                                if (hoy.isBefore(fechaUnidad.getFechaCierre())) {
                                        return ResponseEntity.badRequest().body(
                                                        "No es posible registrar la calificación antes de la fecha de cierre de la unidad.");
                                }
                        } else {
                                return ResponseEntity.badRequest().body(
                                                "No se encontraron fechas registradas para esta unidad y asignación.");
                        }

                        if (fechaUnidadOpt.isPresent()) {
                                FechasUnidades fechaUnidad = fechaUnidadOpt.get();
                                LocalDate fechaCierre = fechaUnidad.getFechaCierre();
                                LocalDate limite = calcularCincoDiasHabiles(fechaCierre);

                                LocalDate hoy = LocalDate.now();
                                if (hoy.isAfter(limite)) {
                                        return ResponseEntity.badRequest()
                                                        .body("El periodo para registrar la calificación ha expirado.");
                                }
                        ...
                        if (existente.isPresent() && existente.get().isRecuperacion()) {
                                return ResponseEntity.badRequest().body("El alumno con matrícula "
                                                + calificacionDTO.getMatriculaAlumno()
                                                + " ya está en proceso de recuperación y no se puede modificar la calificación.");
                        }

                        if (existente.isPresent() && existente.get().isSinDerecho()) {
                                return ResponseEntity.badRequest().body("El alumno con matrícula "
                                                + calificacionDTO.getMatriculaAlumno()
                                                + " ya no tiene derecho a una calificación.");
                        }

                        // Comentar esta condifcion si se requiere habilitar la modificacion de
                        // calificaciones

                        if (existente.isPresent()) {
                                // Asumiendo que getCalificacionOrdinaria() devuelve un tipo numérico como
                                // Double
                                Double calificacionOrdinaria = existente.get().getCalificacionOrdinaria();
                                if (calificacionOrdinaria != null) {
                                        return ResponseEntity.badRequest().body("El alumno con matrícula "
                                                        + calificacionDTO.getMatriculaAlumno()
                                                        + " ya tiene una calificación cargada y no se puede modificar.");
                                }
                        }
                        ...
                        if (porcentajeAsistencias < 80 && porcentajeAsistencias >= 70) {
                                CalificacionesUnidad calificacion = existente
                                                .orElse(new CalificacionesUnidad());
                                ...
                                calificacion.setCalificacionOrdinaria(0.0); // Registrar 0 en calificación
                                                                            // ordinaria
                        ...
                        List<UnidadesPorAsignatura> todasLasUnidades = unidadesAsignaturaRepository
                                        .findByAsignaturas_IdAsignaturas(
                                                        asignacionAsignatura.getAsignaturas()
                                                                        .getIdAsignaturas());
                        ...
                        if (unidadesEnRecuperacion >= unidadesPermitidasParaRecuperacion
                                ...
                                calificacion.setCalificacionOrdinaria(0.0);
                                calificacion.setRecuperacion(false);
                                calificacion.setSinDerecho(true);
                        ...
                        if (calificacionDTO.getCalificacionOrdinaria() < 8) {
                                ...
                                CalificacionResponseDTO responseDTO = convertirAResponseDTO(
        ...
        private CalificacionFinalResponseDTO convertirACalificacionFinalResponseDTO(Calificaciones calificacion) {
                ...
                dto.setIdCalificaciones(calificacion.getIdCalificaciones());
                ...
                dto.setFirmaDocente(calificacion.isFirmaDocente());
                return dto;
        }

        public ResponseEntity<?> registrarCalificacionRemedial(CalificacionRemedialDTO calificacionRemedialDTO) {
                try {
                        Calificaciones calificacion = calificacionesRepository
                                        .findByAlumno_MatriculaAndAsignacionAsignatura_IdAsignacionAsignatura(
                                                        calificacionRemedialDTO.getMatriculaAlumno(),
                                                        calificacionRemedialDTO.getIdAsignacionAsignatura())
                                        .orElseThrow(() -> new ResourceNotFoundException(
                                                        "Calificación no encontrada para alumno y asignatura especificados"));

                        if (!calificacion.isRemedial()) {
                                return ResponseEntity.badRequest().body("La calificación no está en estado remedial.");
                        }

                        calificacion.setCalificacionRemedial(calificacionRemedialDTO.getCalificacionRemedial());
                        Calificaciones actualizada = calificacionesRepository.save(calificacion);

                        CalificacionRemedialResponseDTO responseDTO = convertirAResponseDTO(actualizada);
                        return ResponseEntity.ok(responseDTO);
                } catch (Exception e) {
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                        .body("Error interno del servidor: " + e.getMessage());
        ...
        private CalificacionRemedialResponseDTO convertirAResponseDTO(Calificaciones calificacion) {
                CalificacionRemedialResponseDTO responseDTO = new CalificacionRemedialResponseDTO();
                responseDTO.setId(calificacion.getIdCalificaciones());
                responseDTO.setMatriculaAlumno(calificacion.getAlumno().getMatricula());
                responseDTO.setIdAsignacionAsignatura(
                                calificacion.getAsignacionAsignatura().getIdAsignacionAsignatura());
                responseDTO.setCalificacionRemedial(calificacion.getCalificacionRemedial());
                return responseDTO;
        }

        public ResponseEntity<?> registrarCalificacionSinCandados(CalificacionDTO calificacionDTO) {
