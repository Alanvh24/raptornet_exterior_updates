...
const Referencia = () => {
  ...
  const verReferencia = () => {
    userService.verReferencia().then((response) => {
  ...
  const descargarPdf = async () => {
    ...
    usuarios.forEach((usuario, index) => {
      firstPage.drawText(usuario.matricula, { x: 140, y: 553, size: 20, font: customFont, color: rgb(0, 0, 0) });
      const nombresDivididos = splitNameIfNeeded(usuario.nombre + " " + usuario.ap_paterno + " " + usuario.ap_materno);
      firstPage.drawText(nombresDivididos[0], { x: 120, y: 508, size: 14, font: customFont, color: rgb(0, 0, 0) });
      if (nombresDivididos.length > 1) {
        firstPage.drawText(nombresDivididos[1], { x: 120, y: 488, size: 14, font: customFont, color: rgb(0, 0, 0) }); // Ajusta la coordenada Y según sea necesario
      }
      const idCarreraDividido = splitTextIfNeeded(usuario.id_carrera_1, MAX_ID_CARRERA_LENGTH);
      firstPage.drawText(idCarreraDividido[0], { x: 240, y: 462, size: 14, font: customFont, color: rgb(0, 0, 0) });
      if (idCarreraDividido.length > 1) {
        firstPage.drawText(idCarreraDividido[1], { x: 240, y: 442, size: 14, font: customFont, color: rgb(0, 0, 0) }); // Ajusta la coordenada Y según sea necesario
      }
      firstPage.drawText("$" + usuario.costo, { x: 190, y: 415, size: 20, font: customFont, color: rgb(0, 0, 0) });
      const cuotaDividida = splitTextIfNeeded(usuario.cuota, MAX_CUOTA_LENGTH);
      firstPage.drawText(cuotaDividida[0], { x: 220, y: 365, size: 20, font: customFont, color: rgb(0, 0, 0) });
      if (cuotaDividida.length > 1) {
        firstPage.drawText(cuotaDividida[1], { x: 220, y: 345, size: 20, font: customFont, color: rgb(0, 0, 0) }); // Ajusta la coordenada Y según sea necesario
      }

      firstPage.drawText(usuario.referencia, { x: 155, y: 228, size: 20, font: customFont, color: rgb(0, 0, 0) });
      firstPage.drawText(usuario.fecha_limite, { x: 265, y: 183, size: 20, font: customFont, color: rgb(0, 0, 0) });
    });
  ...
  if (user && user.roles.includes('ROLE_ASPIRANTE') && user.asp_etapa === '2') {
    return (
